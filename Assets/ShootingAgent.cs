...
public class ShootingAgent : Agent
{
    ...
    private void Shoot()
    {
        ...
        GameObject currentBullet = Instantiate(projectile, shootingPoint.position, Quaternion.identity, this.transform.parent.parent);
        if (this.gameObject.CompareTag("mlagent1"))
        {
            currentBullet.tag = "projectile1";
            currentBullet.GetComponent<Projectile>().SetTarget("mlagent2");
        }
        else
        {
        ...
        currentBullet.GetComponent<Projectile>().agent = this;
    ...
    {
        RaycastHit hit;
        if (Physics.Raycast(transform.position, transform.right, out hit, 140))
        {
            if (this.transform.tag == "mlagent1")
            {
                if (hit.transform.tag == "mlagent2")
                {
                    this.AddReward(0.005f);
                }
            }
